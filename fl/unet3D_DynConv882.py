...
class unet3D(nn.Module):
    ...
    def forward(self, input, task_id):
        ...
        x_feat = self.GAP(x)
        # print(x_feat.shape, task_encoding.shape)
        # set_trace()
        ...
        weight_nums, bias_nums = self.prepare_dynamic_weights()
    ...
    def prepare_dynamic_weights(self):
    ...
    def forward_all_task(self, input, task_id): # all task
        ...
        x_fusion = self.fusionConv(x)

        # x8
        x = self.upsamplex2(x_fusion)
        if x.shape != skip3.shape:
            x = F.interpolate(x, size=skip3.shape[-3:], align_corners=True, mode='trilinear')
        x = x + skip3
        x = self.x8_resb(x)
        ...
        # generate conv filters for classification layer
        x_feat = self.GAP(x_fusion)
        n_task = 7
        B = x.shape[0]
        outputs = []
        for tid in range(n_task):
            task_encoding = self.encoding_task(torch.tensor([tid]*B))
            task_encoding = task_encoding.unsqueeze(2).unsqueeze(2).unsqueeze(2)
            # set_trace()
            x_cond = torch.cat([x_feat, task_encoding], 1)
            params = self.controller(x_cond)
