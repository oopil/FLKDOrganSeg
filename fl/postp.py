...
def get_arguments():
    ...
    parser.add_argument("--save_file", type=str, default='outputs/FP/test/')
...
save_file = os.path.join(args.save_file, args.img_folder_path.split('/')[-2] + '_component.txt')
f = open(save_file, 'w')
f.write('Task, Organ_Dice, Tumor_Dice, Organ_HD, Tumor_HD, FP \n')
...
for root, dirs, files in os.walk(args.img_folder_path):
    for i in sorted(files):
        ...
        if label_tumor is not None:
            ...
            dice_c2=-999
            HD_c2=999
        f.write("%s, %f, %f, %f, %f \n" % (i[:-13], dice_c1, dice_c2, HD_c1, HD_c2))
...
print("Sum results")
for t in range(7):
    f.write('Sum: Task%d, Organ_dice, %.4f, Tumor_Dice, %.4f, Organ_HD, %.4f, Tumor_HD, %.4f \n' % (t, val_Dice[t,0], val_Dice[t,1], val_HD[t,0], val_HD[t,1]))
