...
class unet3D(nn.Module):
    def __init__(self, layers, num_classes=3, weight_std = False):
        ...
        encoder_list = [Encoder([1, 2, 2, 2, 2], num_classes, weight_std) for _ in range(7)]
        self.encoder_list = nn.ModuleList(encoder_list)
        self.decoder = Decoder([1, 2, 2, 2, 2], num_classes, weight_std)
    ...
    def forward(self, input, task_id):
        ...
        for i, tid in enumerate(task_id):
            ft, skips = self.encoder_list[tid](input[i:i+1])
            logit = self.decoder(ft, skips)
            logits.append(logit)
        logits = torch.cat(logits, dim=0)        
        return logits

def UNet3D(num_classes=1, weight_std=False):
    print("Using Separate Encoder 8,8,2")
