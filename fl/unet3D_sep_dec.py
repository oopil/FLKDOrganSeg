...
class unet3D(nn.Module):
    def __init__(self, layers, num_classes=3, weight_std = False):
        ...
        decoder_list = [Decoder([1, 2, 2, 2, 2], num_classes, weight_std) for _ in range(7)]
    ...
    def forward(self, input, task_id):
        ...
        x, skip0, skip1, skip2, skip3 = self.encode(input)
        # set_trace()
        logits = []
        for i, id in enumerate(task_id):
            logit = self.decoder_list[id](\
                x[i:i+1],
                [skip0[i:i+1], 
                skip1[i:i+1], 
                skip2[i:i+1], 
    ...
    def forward_all_task(self, input, task_id):
        ...
        for decoder in self.decoder_list:
            outputs.append(decoder(x,[skip0, skip1, skip2, skip3]))
...
def UNet3D(num_classes=1, weight_std=False):
    print("Using Separate Decoder 8,8,2")
