...
class unet3D(nn.Module):
    def __init__(self, layers, num_classes=3, weight_std=False):
        ...
        self.conv1 = conv3x3x3(
            1, 32, stride=[1, 1, 1], weight_std=self.weight_std)

        self.layer0 = self._make_layer(
            NoBottleneck, 32, 32, layers[0], stride=(1, 1, 1))
        self.layer1 = self._make_layer(
            NoBottleneck, 32, 64, layers[1], stride=(2, 2, 2))
        self.layer2 = self._make_layer(
            NoBottleneck, 64, 128, layers[2], stride=(2, 2, 2))
        self.layer3 = self._make_layer(
            NoBottleneck, 128, 256, layers[3], stride=(2, 2, 2))
        self.layer4 = self._make_layer(
            NoBottleneck, 256, 256, layers[4], stride=(2, 2, 2))

        self.fusionConv = nn.Sequential(
            nn.GroupNorm(16, 256),
            nn.ReLU(inplace=in_place),
            conv3x3x3(256, 256, kernel_size=(1, 1, 1), padding=(
                0, 0, 0), weight_std=self.weight_std)
        )

        self.upsamplex2 = nn.Upsample(scale_factor=2, mode='trilinear')

        self.x8_resb = self._make_layer(
            NoBottleneck, 256, 128, 1, stride=(1, 1, 1))
        self.x4_resb = self._make_layer(
            NoBottleneck, 128, 64, 1, stride=(1, 1, 1))
        self.x2_resb = self._make_layer(
            NoBottleneck, 64, 32, 1, stride=(1, 1, 1))
        self.x1_resb = self._make_layer(
            NoBottleneck, 32, 32, 1, stride=(1, 1, 1))
    ...
    def forward(self, input, task_id):
        ...
        x = self.ext_feature(input)

        # set_trace()
        head_inputs = self.precls_conv(x)
        N, _, D, H, W = head_inputs.size()
        logits = self.heads[task_id[0]](head_inputs)
        # logits = [self.heads[id](head_inputs[i:i+1]) for i,id in enumerate(task_id)]
        # logits = torch.cat(logits, dim=0)
        # logits = self.heads[task_id](head_inputs)
        return logits
    ...
    def forward_all_task(self, input, task_id):
        ...
        for head in self.heads:
            outputs.append(head(head_inputs))

        logits = outputs[task_id[0]]
        # logits = self.heads[task_id[0]](head_inputs)

        return logits, outputs

    def ext_feature(self, input):
        x = self.conv1(input)
        x = self.layer0(x)
        skip0 = x
        ...
        if x.shape != skip3.shape:
            x = F.interpolate(
                x, size=skip3.shape[-3:], align_corners=True, mode='trilinear')
        x = x + skip3
        x = self.x8_resb(x)
        ...
        if x.shape != skip2.shape:
            x = F.interpolate(
                x, size=skip2.shape[-3:], align_corners=True, mode='trilinear')
        x = x + skip2
        x = self.x4_resb(x)
        ...
        if x.shape != skip1.shape:
            x = F.interpolate(
                x, size=skip1.shape[-3:], align_corners=True, mode='trilinear')
        x = x + skip1
        x = self.x2_resb(x)
        ...
        if x.shape != skip0.shape:
            x = F.interpolate(
                x, size=skip0.shape[-3:], align_corners=True, mode='trilinear')
        x = x + skip0
        x = self.x1_resb(x)
        return x
