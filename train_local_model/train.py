...
from unet3D_singlehead_bn import UNet3D
# from unet3D_singlehead import UNet3D
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        trainloader, train_sampler = engine.get_train_loader(
            MOTSDataSet(args.data_dir, args.train_list, max_iters=args.itrs_each_epoch * args.batch_size, crop_size=input_size, scale=args.random_scale, mirror=args.random_mirror, target_task=args.target_task), 
        ...
        for epoch in range(args.num_epochs):
            ...
            if (epoch >= 0) and (args.local_rank == 0) and (((epoch % 100 == 0) and (epoch >= 800)) or (epoch % 100 == 0)):
                print('save model ...')
                if args.FP16:
                    checkpoint = {
                    ...
                    torch.save(model.state_dict(),osp.join(args.snapshot_dir, 'MOTS_multihead_' + args.snapshot_dir.split('/')[-2] + '_e' + str(epoch) + '.pth'))
            ...
            if (epoch >= args.num_epochs - 1) and (args.local_rank == 0):
                ...
                if args.FP16:
                    ...
                    torch.save(model.state_dict(),osp.join(args.snapshot_dir, 'MOTS_multihead_' + args.snapshot_dir.split('/')[-2] + '_final_e' + str(epoch) + '.pth'))
                break
